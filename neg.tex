...
% Matrices are bold %RZK0 define domain, define bars over fragment indices
...
\bea
\psi_{xi} = |\chi_{\bar{x}\mu}\rangle {T^{\bar{x}\mu}}_{xi}
\label{eq:LMO}
\eea
...
%
\bea
\hat{I}_{\bar{x}} = |\chi_{\bar{x}\mu}\rangle S^{\bar{x}\mu,\bar{x}\nu} \langle \chi_{\bar{x}\nu} |
\eea
%
...
%
\bea
{G_{\bar{x}\mu}}^{xi} = 2 s_0 [(\mathbf{I}-\mathbf{SR}) \mathbf{H}_{\text{KS}} \mathbf{T}\sigma^{-1}]{_{\bar{x}\mu}}^{xi}
\eea
%
and a single inversion of a [domain-]preconditioner
%
\bea
P_{\bar{x}\mu,\bar{x}\nu} &=& \delta_{xy}[(I_x-S^xR^x) \mathbf{H}_{\text{KS}}(I^x-R^xS^x) \nonumber \\
...
It is worth noting that the problem of inexact invariance of the energy to the  described by Goedecker in Ref.~\onlinecite{goedecker1999linear} is not present in our case because of the properly idempotent density matrix in Eq.~(\ref{eq:dm}).
