...
\caption{The maximun norm of the energy gradient for the PBE/DZVP basis~\cite{vandevondele2007gaussian} optimization of the CLMOs for the hexagonal wurtzite CdSe lattice. $R_c = 3.5$~{\AA}. Modes with the preconditioner eigenvalues lower than 0.05~a.u. are removed from the optimization. The difference in the ground state energy for the two methods is 0.03 kJ/mol per Cd atom.}
%RZZK: Explain the legend in the caption. 
%RZK: I have doubts about accuracy of the reference --- the two-stage procedure is not converged.
...
All matrix multiplications are performed with the DBCSR library~\cite{borvstnik2014sparse} designed for parallel linear-scaling handling of sparse matrices. %RZK: Update the DBCSR reference.
...
%RZK: Letters A., B., C. in the figure
...
%RZK: Unless there is a good reson to minimize the number of figures put insets into a separate figure.
\label{fig:accuracy}
\end{figure}
...
%RZK: replace "OT(MD)" with "Delocalized electrons (MD)". Replace "Low-curvature projector(MC)" with "LCP (MC)"
%Specify the functional, basis set, R_c, \Lambda_c
\label{fig:mc}
\end{figure}
...
%RZK format figure: "Pot" -> "Potential energy" in both instances, "Cons Qty" -> "Conserved Quantity". Set zero energy in the conserved quantity at the first step and put the Cons Qty on the left axis. Make sure that both energy axis span the same energy range. It is desirable to set zero for potential energy to center the black curve vertically. It will be easier to see the magnitude of the drift compared to the pot energy. Also make sure the graph does not have large empty margins along the y-axiz. 
\label{fig:md}
\end{figure}
