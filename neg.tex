...
%The origins of the have not been described. 
...
%
Such orbitals are known under different names --- localized wave functions~\cite{ordejon1995linear}, nonorthogonal Wannier functions~\cite{weitao,ZZZ}, absolutely localized orbitals~\cite{stoll1980use}, orbitals on compact support~\cite{ZZZ}, non-orthogonal localized molecular orbital~\cite{weitao}. They will be referred to as compact molecular orbitals (CMOs) here to emphasize that the expansion coefficients of these orbitals in some localized basis set (e.g. atomic orbitals, Gaussian orbitals, ZZZ) are constrained to be precisely zero outside orbital's predefined localization region.
%
Unfortunately, the development of promising CMO-based LS methods has been hindered~\cite{a:weitao-yang-2013,a:ls-tsuchida-aomm, fattebert-recent} because of inherently difficult variational optimization of CMOs~\cite{a:ls-mauri-galli-car-1993,ordejon1995linear,a:ls-rev-1999, fattebert2004linear, peng2013effective, a:ls-tsuchida-aomm, weitao-yang}. 
%There are also other undisirable features, manifesting depending on a particular method: multiple minima, slow convergence with large number of iterations, position of localization centers must be chosen a priori, runaway solutions, charge-conservation issues. These are not exhibited by DM-based methods (Section 3.E of Ref.~\onlinecite{goedecker1999linear}).
%
%RZK: Complete CMO references:
...
In addition to poor convergence, a straightforward optimization of CMOs, which cannot be constrained to stay both compact and orthogonal~\cite{stoll,ZZZ}, often leads to a ``collapsed'' electronic state represented by linearly dependent occupied orbitals~\cite{ordejon1995linear}.
%
...
Figure~\ref{fig:det} shows that such a collapse, indicated by the vanishing determinant of the CMO overlap matrix, is associated with 
%---caused by or causing---
the convergences problem. 

Several/Numerous efforts were made to resolve these problems. A series of work have focused on using new energy functionals~\cite{mauri1993orbital,kim1995total,ordejon1995linear} that avoid using the inverse of CMO overlap matrix. 
...
Why these attempts were not successful? The origins of the have not been described. Dramatically increased computational cost is one reason.
