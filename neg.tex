...
\title{Robust optimization/construction of compact[ified] orbitals}
...
\begin{abstract}
Density functional theory based on compact nonorthogonal molecular orbitals is a conceptually simple method that can enable low-cost linear-scaling modeling of the electronic structure of molecules and materials with finite band gap. 
Unfortunately, its development has been hindered because a compact representation of the electronic ground state is difficult to find [in a variational optimization procedure]. 
In this work, we identified the origin of the slow and unstable convergence and developed a simple and robust linear-scaling optimization procedure with a low computational overhead. 
% RZK0: greater detail in the abstract?
%In this work, we identified the origin of problem as that occupied and nearly-occupied mixing modes and developed a robust optimization procedure to remove these modes from the optimization procedure.  using the approximate Hessian. We show that, by removing these directions, the energy and other properties can be obtained accurately and the computational cost is significantly reduced. 
We demonstrated the efficiency of the method for a variety of systems ranging from molecular liquids to semi-conductors and insulators.  
...
% RZK: all of the methods below perform DM optimization.
They include the Fermi operator expansion~\cite{goedecker1994efficient,goedecker1995tight}, divide-and-conquer~\cite{yang1991direct,yang1991local}, and direct DM optimization methods~\cite{li1993density, vandevondele2012linear}. 
...
From the computational point of view, a direct variation of compact molecular orbitals (CMOs)---orbitals that are strictly localized within predefined spatial regions---is preferable because LS can be achieved with significantly fewer variables. 
The computational advantages of this orbitals-only LS DFT are especially pronounced in accurate calculations that require many basis functions per atom. 
The use of CMOs is also advantageous from the physical point of view because CMOs provide clear, chemically meaningful, transferable description of interactions between atoms or molecules~\cite{ZZZweitao,stoll,khaliullin2007unravelling,khaliullin2008analysis}.
Unfortunately, the development of promising orbital-based LS methods has been all but abandoned~\cite{a:weitao-yang-2013,a:ls-tsuchida-aomm} because of the difficult variational optimization of CMOs~\cite{a:ls-rev-1999,a:ls-mauri-galli-car-1993,ordejon1995linear,fattebert2004linear,peng2013effective,a:ls-tsuchida-aomm}. 
%RZK: 
In addition to poor convergence, a straightforward optimization of CMOs, which cannot be constrained to stay orthogonal~\cite{stoll,ZZZ}, almost inevitably leads to a ``collapsed'' electronic state represented by linearly dependent orbitals~\cite{ordejon1995linear}.
%
...
Figure~\ref{fig:det} shows that such a collapse, indicated by the vanishing determinant of the CMO overlap matrix, is associated with---caused by or causing---the convergences problem. 

Several efforts were made to resolve these problems. A series of work have focused on using new energy functionals~\cite{mauri1993orbital,kim1995total,ordejon1995linear} that avoid using the inverse of CMO overlap matrix. 
Utilization of unoccupied orbitals~\cite{galli1995?} or extra optimization variables~\cite{burger2008linear,peng2013effective} has been proposed to mitigate convergence problems. 
Why these attempts were not successful? Dramatically increased computational cost is one reason.

The convergence problem has been [reliably] solved for weakly-interacting molecular systems. Tsuchida \emph{et al.} showed~\cite{tsuchida2007augmented,tsuchida2008ab} that CMOs can be optimized efficiently if they are forced to be orthogonal to precomputed auxiliary tightly localized nonoverlaping orbitals fixed on their molecules. Similar Recently, in Ref.~\onlinecite{khaliullin2013efficient}, a block-diagonal reference is first calculated and then correction which is constrained to be orthogonal to the reference is added in the second step. These two methods have all been successful in improving the convergence by constraining the MOs to be orthogonal to some reference states, thus requiring the reference to be somewhat physically meaningful. RZK: this only works for weakly-interacting molecular systems (the DOFs projected out already resemble the final occupied states)
