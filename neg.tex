...
%RZK:  Explain legend in the caption. I have doubts about accuracy of the reference --- the two-stage procedure is not converged.
...
All parallel sparse matrix multiplications are performed with the DBCSR library~\cite{a:ls-dm-sign}. %RZK: Update DBCSR reference.
The diagonalization of the preconditioner matrices is done independently for each domain maintaining the LS behavior of the method. 
A special care was taken to reduce the computational overhead of the optimization procedure for large Gaussian basis sets. 
To this end, the order of matrix multiplications is chosen to avoid steps that scale cubically with the size of the Gaussian basis set. 
...
\caption{CLMO optimization timing benchmark for the hexagonal CdSe lattice. The PBE/DZVP-MOLOPT basis calculation with $R_c=3.5$~{\AA} and $\Lambda=ZZZ$~a.u. is done on 256 compute cores.}
\label{fig:scaling}
\end{figure}
...
We develop a new DFT method based on LMOs. The notorious convergence problem is avoided by identifying the small but none-zero eigenvalues of the Hessian [while obviating a computationally costly construction and diagonalization of the Hessian]. These directions are projected out from the gradient and the preconditioner, and the remaining gradient shows fast convergence with PCG. This projection can be understood as a relaxed orthogonality constrain for the CLMOs. This method works even when the interaction and covalency between fragments are strong. We demonstrate the accuracy and efficiency of the method on systems like diamond silicon, CdSe. We also show a MD simulation in protonated water nanoclusters, and show that it can deal with systems where chemical reactions happen.
